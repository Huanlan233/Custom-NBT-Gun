# 召唤箭矢，CustomPotionEffects中的幸运(Id:26b)效果负责检测中箭，速度(Id:1b)效果的等级负责存储伤害数量
...
execute as @e[tag=cng.bullet_not_fired] at @s store result entity @s CustomPotionEffects[1].Duration int 1 run scoreboard players get @a[limit=1,distance=..2.5,sort=nearest] cng.damage
